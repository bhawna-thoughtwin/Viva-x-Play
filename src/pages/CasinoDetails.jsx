...
const allGameNames = [
  'Wanted Dead or Wild', 'Cash Quest', 'San Quentin Xways', 'Fighter Pit',
  'Dragon Gold 88', 'Aztec Clusters', 'Chicken Man', 'Juicy Fruits Multihold',
  'Book of Time', 'Sweet Bonanza', 'Gates of Olympus', 'Sugar Rush',
  'Fortune Tiger', 'Wild West Gold', 'Big Bass Splash', 'Fruit Party',
  'Fire Joker', 'Rise of Merlin', 'Money Train 3', 'Razor Shark',
  'Starlight Princess', 'Wolf Gold', 'Reactoonz', 'Mental',
];

const categoryMeta = {
  slots: { label: 'Slots', icon: slotsIcon },
  roulette: { label: 'Roulette', icon: rouletteIcon },
  blackjack: { label: 'Blackjack', icon: blackjackIcon },
  poker: { label: 'Poker', icon: pokerIcon },
  crash: { label: 'Crash', icon: crashIcon },
  baccarat: { label: 'Baccarat', icon: baccaratIcon },
  jackpot: { label: 'Jackpot', icon: jackpotIcon },
...
const CasinoDetails = () => {
  ...
  const meta = categoryMeta[category] || { label: category, icon: slotsIcon };
  ...
  return (
    <div className="w-full min-h-screen bg-[#ECECEC]">
      <div className="flex flex-col gap-4 md:gap-5 p-3 md:p-6 ml-[-30px] max-md:-ml-[40px]">
        ...
        {/* ── Single header row: back · icon · title · search · filter · sort ── */}
          ...
          <div className="flex items-center gap-3">
            ...
            {meta.icon && (
              <img
                src={meta.icon}
                alt={meta.label}
                className="w-[24px] h-[24px] md:w-[26px] md:h-[26px] object-contain shrink-0"
              />
            )}
          ...
          <div className="flex flex-col sm:flex-row md:items-center gap-3 md:ml-auto w-full md:w-auto">
            ...
            <div className="relative w-full sm:w-auto">
              ...
              >
                ...
                <span>
                  Sort by: <span className="text-[#1CD4FF]">{sortBy}</span>
                </span>
                ...
                <div className="absolute top-[40px] right-0 z-50 bg-white border border-[#e5e7eb] rounded-xl shadow-lg overflow-hidden min-w-[160px] w-full sm:w-auto">
                  {SORT_OPTIONS.map((opt) => (
                    <button
                      ...
                      className={`w-full text-left px-4 py-3 text-[14px] hover:bg-[#f0fdff] transition-colors ${sortBy === opt
                          ? 'text-[#1CD4FF] font-semibold bg-[#f0fdff]'
                          : 'text-[#333]'
                        }`}
                    >
                      {opt}
